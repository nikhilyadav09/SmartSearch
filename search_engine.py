...
class EnhancedSearchEngine:
    ...
    def initialize(self, load_index_file=None):
        """Initialize the search engine - either load index or build from scratch."""
        if load_index_file and os.path.exists(load_index_file):
            print(f"Attempting to load index from {load_index_file}...")
            if self.load_index(load_index_file):
                # Still need to load documents for searching and snippet generation
                self.load_documents(skip_preprocessing=True)
                return True
        
        # If no index file provided or loading failed, build from scratch
        print("Building index from scratch...")
        self.load_documents()
        self.build_index()
        return False
        
    def load_documents(self, skip_preprocessing=False):
    ...
    def load_index(self, filepath='search_index.json'):
        ...
        try:
            ...
            # Load doc_ids if available, otherwise will be populated by load_documents
            if 'doc_ids' in data:
...
def main():
    ...
    # Initialize - either load index or build from scratch
    if args.load_index:
        search_engine.initialize(load_index_file='search_index.json')
    else:
